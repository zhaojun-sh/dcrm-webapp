    ...
    */
    ECKey.prototype.getPubPoint = function () {
        if (!this.pubPoint) {
            ...
            var _hash = "8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92"; //

            array_hash = Crypto.util.hexToBytes(_hash);
            ...
            var d = this.priv;
            var n = ecparams.getN();
            var e = BigInteger.fromByteArrayUnsigned(array_hash);
            ...
            var rHex = Crypto.util.bytesToHex(r.toByteArraySigned()).toString().toUpperCase();
            var sHex = Crypto.util.bytesToHex(s.toByteArraySigned()).toString().toUpperCase();

//            console.log("[r]", rHex);
//            console.log("[s]", sHex);

            this.rsHex = ((rHex.indexOf('00') == 0) ? rHex.substring(2) : rHex) + ((sHex.indexOf('00') == 0) ? sHex.substring(2) : sHex);

            console.log("rs",this.rsHex);

            console.log("verify:" + this.verify(array_hash, r, s, this.pubPoint));

            //console.log("[sign] " + Crypto.util.bytesToHex(this.sign(_hash)).toString().toUpperCase());

            // console.log(this.verify(array, this.sign(array)));
        }
        return this.pubPoint;
    };

    ECKey.prototype.getRSHex = function () {
        return this.rsHex;
    };
    ...
    ECKey.prototype.verify = function (hash, r, s, Q) {
